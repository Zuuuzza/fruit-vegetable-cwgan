...
LATENT_DIM = 100  # rozmiar wektora szumu
...
LAMBDA_GP = 10  # waga gradient penalty (regularyzacja)
SAVE_IMAGE_INTERVAL = 15 
LR = 1e-4  # learning rate
BATCH_SIZE = 64  # rozmiar batcha
CRITIC_ITERATIONS = 5  # ile razy uczymy krytyk na jedną aktualizację generatora
EMBED_DIM = 100  # wymiar wektora etykiety
FEATURES_GEN = 64  # liczba cech w generatorze
FEATURES_CRITIC = 64  # liczba cech w krytyku

SAMPLES_FOLDER = "Owoce_wyniki/obrazki_cwganGP"
MODELS_FOLDER = "Owoce_wyniki/model_cwganGP"
DATA_PATH = "./owoce"
...
# transformacja obrazów: zmiana rozmiaru, konwersja do tensora i normalizacja do [-1, 1]
...
# pobieranie danych z folderu i prztransformowanie ich do Tensora zdefined wcześniej
...
class cGenerator(nn.Module):
    def __init__(self, z_dim, channels_img, features_g, embed_dim, num_classes):
        ...
        self.net = nn.Sequential(
            ...
            nn.Tanh(),  # przeskalowanie pikseli do [-1, 1]
        )
    ...
    def forward(self, noise, labels):
        # łączenie szumu z zakodowaną etykietą
...
def gradient_penalty(critic, real, fake, labels):
    ...
    gp = ((gradient.norm(2, dim=1) - 1) ** 2).mean()  # gradient penalty
    return gp

# zapisywnaie obrazków
...
# trenowanie
def train_cwgan(gen, critic, loader, epochs, lr, critic_iters):
    # używamy optymalizatorów Adam
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (real, labels) in enumerate(loader):
            ...
            # krytytk aktualizowany kilka razy
            for _ in range(critic_iters):
                ...
                # obliczanie gradient penalty
                gp = gradient_penalty(critic, real, fake.detach(), labels)

                # funkcja kosztu krytyka (WGAN loss + gradient penalty)
                loss_critic = -(crit_real.mean() - crit_fake.mean()) + LAMBDA_GP * gp

                opt_critic.zero_grad()
                loss_critic.backward()
                opt_critic.step()

            # generator aktualizowany raz
            z = torch.randn(cur_batch_size, LATENT_DIM, device=device)
            fake = gen(z, labels)
